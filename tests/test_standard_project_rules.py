...
class StandardProjectRulesTests(unittest.TestCase):
 def test_rules_copy_covers_global_contract(self):
  ...
  for token in ('primary `main` checkout','CodeGraph','single `TODO.md`','dependencies, order, completion criteria and tests','reread global memory rules','smallest useful MVP','no subagents','root cause','commit immediately','scenario matrix','total, scenario, and category','/home/public/HermesVault/Automation/','validate-text-encoding','UTF-8','Never store secrets','full tests','project memory'):
