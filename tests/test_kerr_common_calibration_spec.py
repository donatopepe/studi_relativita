...
class Spec(unittest.TestCase):
 def test_contract(self):
  ...
  todo=T.read_text();self.assertIn('Kerr shared-calibration robustness MVP',todo);self.assertIn('1. [x] Ratify shared-calibration robustness specification.',todo);self.assertIn('5. [ ] Closure and publication. **ACTIVE — local gates green; push/CI pending**',todo)
