...
class Spec(unittest.TestCase):
 def test_contract(self):
  ...
  self.assertIn('Metric:** `8/8`',p);self.assertIn('Kerr bounded reference-drift robustness MVP',t);self.assertIn('1. [x] Ratify bounded reference-drift specification.',t);self.assertIn('2. [x] Extend authoritative Kerr scenario matrix and runner.',t);self.assertIn('3. [x] Implement reference-drift robustness engine.',t);self.assertIn('4. [x] Generate stable artifacts and bilingual scientific record.',t);self.assertIn('5. [x] Closure and publication.',t)
