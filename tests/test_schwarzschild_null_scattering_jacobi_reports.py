...
class ScatteringJacobiReportTests(unittest.TestCase):
 ...
 def test_bilingual_reports_have_exact_authority_and_values(self):
  ...
  for token in (s.STATUS,s.SCOPE,s.GATE,s.CLASSIFICATION,'rank_shape_boundary = 2','rank_with_log_M = 2','log_M_column_norm = 5.724578281939044e-09','scale_null_direction = [0, 0, 1]','UMCH = UNPROVEN','detection = NO_POSITIVE_DETECTION_CLAIM','structural_dead_end = NOT_DECLARED','DIRECT_REVIEW_NO_SUBAGENT'):
