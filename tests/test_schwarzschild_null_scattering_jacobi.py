...
class SchwarzschildNullScatteringJacobiTests(unittest.TestCase):
 def test_profile_preserves_branches_screen_and_vacuum_tidal_structure(self):
  ...
  anchor=s.profile_control(M=1.,rho=3.000001,R=4.,orientation=1,n=40)['samples']
  near=next(z for z in anchor if z['branch']=='turning')['K']
  self.assertAlmostEqual(near[0][0],-1/3,places=5)
  self.assertAlmostEqual(near[1][1],1/3,places=5)
