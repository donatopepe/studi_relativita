...
class KerrJacobiSourcesAndScenarios(unittest.TestCase):
    ...
    def test_manifest_covers_all_preregistered_scenarios_and_categories(self):
        ...
        self.assertEqual([item["id"] for item in data["scenarios"]], [f"J{index:02d}" for index in range(1, 87)])
        self.assertEqual(set(item["category"] for item in data["scenarios"]), {"conformance", "orientation", "scale", "preparation", "analyzer", "receiver", "likelihood", "noise", "calibration", "robustness", "asymptotic", "profiling", "calibration_channel", "reference_placement", "reference_drift", "calibration_mismatch", "finite_sample", "estimated_mean"})
    ...
    def test_runner_supports_total_granular_category_and_json_report(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(payload["summary"], {"PASS": 86, "FAIL": 0, "SKIP": 0, "BLOCKED": 0})
            ...
            category = subprocess.run(["python", str(RUNNER), "--mode", "granular", "--category", "estimated_mean"], text=True, capture_output=True)
            self.assertEqual(category.returncode, 0, category.stdout + category.stderr)
            self.assertTrue(all(item["category"] == "estimated_mean" for item in json.loads(category.stdout)["results"]))
