...
10. Before creating any script or tool, search `/home/public/HermesVault/Automation/`; reuse or compatibly extend its catalog first. Add a new tool only for distinct behavior, with documented interface, fail-closed behavior, no secrets, and tests/dry-run. Review catalog after each activity for reusable additions.
11. Preserve deterministic artifacts, explicit thresholds, negative results, provenance, source scope, and scientific nonclaims.
12. Keep text UTF-8. Use shared `/home/public/HermesVault/Automation/bin/validate-text-encoding`; auto-fix only unambiguous reversible cases with shared cleaner.
13. Never store secrets in repository, CodeGraph, logs, artifacts, or Hermes memory.
14. Before completion: focused and full tests, total and granular scenario runs, deterministic checks, `git diff --check`, shared UTF-8 validation, CodeGraph sync, clean status, remote CI when published.
15. Update this file and project memory whenever global rules change; verify propagation through tests.
