...
The exact Brinkmann four-dimensional Levi-Civita loop control now reaches genuine connection-derived null-rotation holonomy. Raw loop vectors are nontrivial and orientation-labelled, but every ordinary spectrum is unipotent, same-base compositions are Abelian, profile histories can collide in holonomy while differing in Jacobi phase map, and the affine orbit remains scale blind. Status `EXACT_PLANE_WAVE_LEVI_CIVITA_NULL_ROTATION_HOLONOMY_RAW_LOOP_VECTOR_NONTRIVIAL_SPECTRUM_UNIPOTENT_ABELIAN_AND_AFFINE_SCALE_BLIND_NOT_ELL0`. Physical causal loops, tetrad anchor, null normalization, detector readout, generic non-Abelian spacetime holonomy, and an `ell0` law remain open.
