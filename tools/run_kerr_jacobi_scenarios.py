...
def main(argv=None):
 p=argparse.ArgumentParser();p.add_argument('--mode',choices=('total','granular'),required=True);p.add_argument('--scenario');p.add_argument('--category',choices=('conformance','orientation','scale','preparation','analyzer','receiver','likelihood','noise','calibration','robustness','asymptotic','profiling','calibration_channel','reference_placement','reference_drift','calibration_mismatch','finite_sample','estimated_mean','temporal_dependence','cross_stream_dependence','pairing_lag','pairing_jitter','latent_pairing_jitter','latent_lag_law_robustness'));p.add_argument('--report-json');a=p.parse_args(argv);data=json.loads(MANIFEST.read_text(encoding='utf-8'));cases=data['scenarios']
 if a.mode=='granular':
