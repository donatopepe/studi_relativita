...
6. **Profile/window collision.** Construct distinct tracefree profile histories with the same loop integral controlling `b_LC`; require distinct sampled profiles but colliding endpoint holonomies. Also retain a comparison where the tidal window `W` collides while Jacobi `P_K` differs, preserving channel distinctions.
