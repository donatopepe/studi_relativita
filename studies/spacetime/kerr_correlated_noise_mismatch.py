...
def threshold_control():
 gp=[common.gamma(1)[i][i]for i in range(2)];gm=[common.gamma(-1)[i][i]for i in range(2)];components=[GAIN_MIN**2*(R[i]-gp[i])for i in range(2)]+[GAIN_MIN**2*(gm[i]-R[i])for i in range(2)];tau=min(components);return {'components':components,'tau':tau,'boundary_component':components.index(tau),'boundary_residual':abs(tau-min(components))}
def safe_mismatch_control():
 tau=threshold_control()['tau'];bound=SAFE*tau;return {'safe_fraction':SAFE,'mismatch_bound':bound,'remaining_margin':tau-bound}
def threshold_collision_control():
 tau=threshold_control()['tau'];D=joint_difference(1,[GAIN_MIN,GAIN_MIN],NOISE);M=[[-tau,0.],[0.,0.]];shifted=[[D[i][j]-M[i][j]for j in range(2)]for i in range(2)];return {'branch':'plus','gain':[GAIN_MIN,GAIN_MIN],'differential_mismatch':M,'shifted_difference':shifted,'eigenvalues':eig2(shifted),'zero_eigenvalue_residual':min(abs(x)for x in eig2(shifted))}
def basis_scale_control(theta,factor):
 tau=threshold_control()['tau'];co=math.cos(theta);si=math.sin(theta);Q=[[co,-si],[si,co]];M=[[-tau,0.],[0.,0.]];rot=receiver.mm(Q,receiver.mm(M,receiver.tr(Q)));basis=max(abs(eig2(rot)[i]-eig2(M)[i])for i in range(2));scaled_tau=factor*factor*tau;scaled=[[factor*factor*x for x in row]for row in M];scale=abs(max(abs(x)for x in eig2(scaled))/scaled_tau-1.);return {'basis_residual':basis,'scale_residual':scale,'scale_null_direction':[1.,0.,0.,0.,0.]}
...
def scenario_control(name,case):
 table={'mismatch_spd':lambda:(spd_control()['anchor_spd']and spd_control()['invalid_rejected'],spd_control()['minimum_eigenvalue'],0.),'mismatch_cancellation':lambda:(shared_noise_cancellation_control()['residual']<2e-10,shared_noise_cancellation_control()['residual'],2e-10),'mismatch_cones':lambda:(sign_cone_control()['opposite_definite'],0.,1.),'mismatch_threshold':lambda:(threshold_control()['tau']>1e-3,threshold_control()['tau'],1e-3),'mismatch_safe':lambda:(safe_mismatch_control()['remaining_margin']>1e-3,safe_mismatch_control()['remaining_margin'],1e-3),'mismatch_collision':lambda:(threshold_collision_control()['zero_eigenvalue_residual']<2e-10,threshold_collision_control()['zero_eigenvalue_residual'],2e-10),'mismatch_basis_scale':lambda:(max(basis_scale_control(.37,2.5)['basis_residual'],basis_scale_control(.37,2.5)['scale_residual'])<2e-10,max(basis_scale_control(.37,2.5)['basis_residual'],basis_scale_control(.37,2.5)['scale_residual']),2e-10),'mismatch_nonclaims':lambda:(not no_ell0_gate()['ell0_identified'],0.,1.)}
 if name not in table:raise KeyError(name)
 return table[name]()
def build_artifact():
 controls=[('spd',spd_control()['anchor_spd']and spd_control()['invalid_rejected'],spd_control()['minimum_eigenvalue'],0.,'minimum'),('cancellation',shared_noise_cancellation_control()['residual']<2e-10,shared_noise_cancellation_control()['residual'],2e-10,'maximum'),('sign_cones',sign_cone_control()['opposite_definite'],0.,1.,'boolean'),('threshold',threshold_control()['tau']>1e-3,threshold_control()['tau'],1e-3,'minimum'),('safe_mismatch',safe_mismatch_control()['remaining_margin']>1e-3,safe_mismatch_control()['remaining_margin'],1e-3,'minimum'),('threshold_collision',threshold_collision_control()['zero_eigenvalue_residual']<2e-10,threshold_collision_control()['zero_eigenvalue_residual'],2e-10,'maximum'),('basis_scale',max(basis_scale_control(.37,2.5)['basis_residual'],basis_scale_control(.37,2.5)['scale_residual'])<2e-10,max(basis_scale_control(.37,2.5)['basis_residual'],basis_scale_control(.37,2.5)['scale_residual']),2e-10,'maximum'),('nonclaims',not no_ell0_gate()['ell0_identified'],0.,1.,'boolean')]
