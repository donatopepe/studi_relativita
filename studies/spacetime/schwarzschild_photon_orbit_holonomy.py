...
def build():
 ...
 raw={'M':1.0,'r_ph':a['r'],'f_ph':1/3,'tetrad':m.tetrad(1,3),'orientation':1,'winding':1,'Delta_phi':a['dp'],'Delta_t':a['dt'],'Delta_tau':a['proper'],'null_tangent':[a['dt']/a['dp'],0,0,1],'null_residual':causal_control()['null_residual'],'geodesic_residual':causal_control()['geodesic_residual'],'Gamma_t':a['gt'],'Gamma_phi':a['gp'],'A_null':a['an'],'A_closure':a['ac'],'T_null':a['tn'],'T_closure':a['tc'],'H_photon':a['tetrad'],'ordered_reverse':o['ordered_reverse'],'H_reverse':r['H_reverse'],'characteristic_coefficients':m.characteristic(a['tetrad']),'spectrum_or_surrogate':{'kind':'CHARACTERISTIC_COEFFICIENTS_NO_NUMPY_EIGENSOLVER','values':m.characteristic(a['tetrad'])},'winding_products':w,'Jacobian_joint':'NOT_APPLICABLE_DISCRETE_WINDING_NO_CONTINUOUS_JACOBIAN','scale_factor':sc['scale_factor'],'scale_orbit':sc['scale_orbit'],'flat_or_large_radius_control':'NO_FINITE_RADIUS_CIRCULAR_NULL_GEODESIC_AT_M_ZERO'}
