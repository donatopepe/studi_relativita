...
DESIGN={'gains':[.8,1.1],'noise':[.25,.35],'reference':[.1,10.],'sample_counts':[1.,10.,100.]}
...
def known_reference_control():
 gp=[common.gamma(1)[i][i]for i in range(2)];gm=[common.gamma(-1)[i][i]for i in range(2)];r=DESIGN['reference'];products=[(gp[i]-r[i])*(gm[i]-r[i]) for i in range(2)];margins=[-x for x in products]
 # Equal joint outputs require a positive squared-gain ratio, possible iff placement product is positive.
 return {'reference':r,'placement_products':products,'obstruction_margins':margins,'positive_collision_possible':any(x>=0. for x in products),'corrected_by':'kerr-calibration-reference-placement-correction-v1'}
